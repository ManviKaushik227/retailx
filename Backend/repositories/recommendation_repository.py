...
#     # 1. Handling Tags: List ko string mein convert karna
#     def clean_tags(x):
#         if isinstance(x, list):
#             return " ".join(x)
#         return str(x) if pd.notnull(x) else ""

#     # 2. Content Mix with Weighting:
#     # Sub-category aur Category ko repeat kiya hai taaki algorithm inpar zyada dhyan de
#     df['content'] = (
#         df['name'].fillna('') + " " + 
...
#     try:
#         # Product index nikalna
#         idx = df.index[df['id'].astype(str) == str(product_id)][0]
        
#         # Similarity scores calculate karna
#         sim_scores = list(enumerate(cosine_sim[idx]))
#         sim_scores = sorted(sim_scores, key=lambda x: x[1], reverse=True)
        
#         # Top N (khud ko exclude karke)
...
# # --- SMART PREFERENCE LOGIC (With Shuffle & Diversification) ---
# def get_preference_recommendations(user_prefs):
#     all_products = get_all_products()
#     if not all_products:
#         return []

#     # 1. CASE: User has selected preferences
...
#                str(p.get('sub_category')).lower() in user_prefs_clean
#         ]
...
#     # 2. CASE: Cold Start (No prefs or no matches)
#     # Pura random dikhane ke bajaye har category se thoda-thoda mix dikhao
#     try:
#         random_picks = random.sample(all_products, min(len(all_products), 12))
#         return random_picks
#     except Exception:
