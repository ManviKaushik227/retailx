...
import { CartContext } from "../context/CartContext"; // Path apne hisab se check kar lena
import confetti from "canvas-confetti";
import axios from "axios";
import { CheckCircleIcon, XCircleIcon, ArrowRightIcon } from "@heroicons/react/24/solid";

const PaymentSuccess = () => {
  // Purane setCart ki jagah ab fetchCart use karenge
  ...
  useEffect(() => {
    const saveOrderToDatabase = async () => {
      ...
      // LocalStorage se temporary data uthana (Checkout page se set kiya hua)
      ...
      try {
        ...
        const res = await axios.post("http://127.0.0.1:5000/api/orders/create", {
          session_id,
          address,
          items,
          userId, // User ID bhejna zaroori hai
          total: Number(total),
        }, {
        ...
        if (res.status === 200 || res.status === 201) {
          // 2. Database mein Cart ko empty karna (Naya API call)
          // Note: Aapko backend mein ek route banana hoga /api/cart/clear jo userId leta ho
          ...
          // 5. Context refresh karna taaki cart icon 0 ho jaye
          await fetchCart(); 
      ...
      <div className="max-w-2xl w-full">
                ...
                <div>
                    ...
                    <div className="flex -space-x-3 overflow-hidden">
                        {orderDetails?.items?.map((item, idx) => (
                            <img key={idx} src={item.imageURL || item.image} alt="" className="inline-block h-12 w-12 rounded-full ring-4 ring-white object-cover bg-gray-100" />
                        ))}
                    </div>
                </div>
        ...
        {status === "error" && (
          <div className="bg-white p-12 rounded-3xl shadow-xl text-center space-y-6">
            ...
            <p className="text-gray-500">The payment was successful, but we couldn't update your order status. Don't worry, our team is on it.</p>
            <button 
              onClick={() => navigate("/")}
