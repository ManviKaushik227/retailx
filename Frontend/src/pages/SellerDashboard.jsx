...
<div className="space-y-4 pt-8 border-t border-slate-100 mt-8">
  <div className="flex justify-between items-center px-4">
    <div>
      ...
      <p className="text-[10px] text-slate-400 font-bold">
        Specific data like Material, Weight, etc.
      </p>
    </div>
  ...
  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
    {Object.entries(newProduct.specs).map(([specKey, specVal]) => (
      <div
        key={specKey}   // ✅ FIXED
        className="flex gap-2 p-4 bg-slate-50/50 rounded-[2rem] border border-transparent hover:border-slate-200 transition-all group"
      >
        <div className="flex-1 space-y-2">
          {/* SPEC LABEL */}
          <input
            placeholder="Label (e.g. Material)"
            value={specKey.startsWith("spec_") ? "" : specKey}
            onChange={(e) => {
              if (!e.target.value) return;

              const updatedSpecs = { ...newProduct.specs };
              const value = updatedSpecs[specKey];
              delete updatedSpecs[specKey];
              updatedSpecs[e.target.value] = value;
          ...
          <tr key={p._id} className="group hover:bg-slate-50/50 transition-colors">
            ...
            <td className="px-10 py-6">
              <div className="flex items-center justify-center gap-4 bg-white border border-slate-200 w-fit mx-auto px-4 py-2 rounded-2xl shadow-sm">
                <button onClick={() => onUpdateStock(p.id, p.stock, -1)} className="text-slate-400 hover:text-red-500 transition-colors"><Minus size={16}/></button>
                <span className="font-black text-slate-900 min-w-[20px] text-center">{p.stock}</span>
                <button onClick={() => onUpdateStock(p.id, p.stock, 1)} className="text-slate-400 hover:text-emerald-500 transition-colors"><Plus size={16}/></button>
              </div>
            </td>
            <td className="px-10 py-6 text-right">
              <div className="flex justify-end gap-2">
                ...
                <button onClick={() => onDelete(p.id)} className="p-3 bg-slate-100 text-slate-600 rounded-xl hover:bg-red-500 hover:text-white transition-all"><Trash2 size={16}/></button>
              </div>
            </td>
          </tr>
        ))}
